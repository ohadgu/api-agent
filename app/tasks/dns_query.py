...
def _resolve(domain: str) -> list[str]:
    ...
    try:
        ...
        # Use set for O(1) duplicate checking, then convert to list
        ip_set = set()
...
def dns_query(domain: str):
    ...
    try:
        ips = _resolve(domain.strip())
        primary = ips[0] if ips else None
        result = {
            "domain": domain.strip(),
            "ips": sorted(ips),  # Sort for consistent output
            "primary": primary,
            "ip_count": len(ips)
        }
        return result
...
def enqueue_dns_query(domain: str, db) -> dict:
    ...
    res = dns_query.apply_async(args=[domain])
    tr = TaskRun(id=res.id, name=dns_query.name, status="PENDING", args_json=[domain])
    db.add(tr)
    db.commit()
    return {"task_id": res.id, "status": "queued", "name": dns_query.name, "domain": domain}
